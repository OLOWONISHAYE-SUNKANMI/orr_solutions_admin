...
export default function BusinessSystemCardsManagement({
  ...
  const fetchCards = async () => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:8000'}/admin-portal/v1/cms/business-system-cards/`, {
        headers: {
  ...
  const handleSave = async (cardId: number, cardData: BusinessSystemCard) => {
    ...
    try {
      const token = typeof window !== 'undefined' ? localStorage.getItem('access_token') : null;
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:8000'}/admin-portal/v1/cms/business-system-cards/${cardId}/`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          ...(token && { 'Authorization': `Bearer ${token}` })
