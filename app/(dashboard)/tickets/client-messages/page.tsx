...
export default function ClientMessagesPage() {
  ...
  const fetchClientTickets = async () => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:8000'}/admin-portal/v1/tickets/?source=manual_request`, {
        headers: {
  ...
  const sendAutoReply = async (ticketId: string, replyType: string = "initial") => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:8000'}/admin-portal/v1/tickets/${ticketId}/auto-reply/`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('access_token')}`,
