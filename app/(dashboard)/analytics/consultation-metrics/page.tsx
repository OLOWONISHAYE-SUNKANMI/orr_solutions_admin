    ...
    const fetchData = async () => {
      try {
        const [consultationRes, schedulingRes] = await Promise.all([
          fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:8000'}/admin-portal/v1/consultation-metrics/performance/`),
          fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:8000'}/admin-portal/v1/consultation-metrics/scheduling-analytics/`)
        ]);
